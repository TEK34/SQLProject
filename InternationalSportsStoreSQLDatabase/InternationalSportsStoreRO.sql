...
select adresa,tel from DEPOZITE;
...
select nume,descriere from DEPARTAMENTE;
...
select denumire,descriere from FUNCTII;
select *from FUNCTII;
...
select Nume,Pren,Tel from ANGAJATI;
select *from ANGAJATI;
 ...
 select nume,descriere from PRODUSE;
 select *from PRODUSE;
...
insert into DEPOZITE_PRODUSE values(24,8,2);
select id_dep,id_prod from DEPOZITE_PRODUSE;
...
select nume,tel from FURNIZORI;
...
select id_prod,cantitate,pret_cumparare from EVIDENTA_APROVIZIONARE;
...
select nume,tel,importanta from MAGAZINE;
...
select data_comenzii,cantitate,pret_vanzare,data_exped from EVIDENTA_VANZARILOR;
select *from EVIDENTA_APROVIZIONARE;
...
select DATA,informatia from INFORMATII_TRIMISE_CLIENTILOR;